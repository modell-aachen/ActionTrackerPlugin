...
my $textonlyfmt = new Foswiki::Plugins::ActionTrackerPlugin::Format(
    "Text", "\$text", "cols", "\$text", "", "");
...
HERE
    ...
    Foswiki::Func::saveTopic($this->{test_web_2},
...
HERE
    ...
    Foswiki::Func::saveTopic($this->{test_web_2}, "SecretTopic", undef, <<HERE);
...
sub testAllActionsInWebTest {
    ...
    $this->assert_not_null($actionees{"$Foswiki::cfg{UsersWebName}.$this->{test_user_wikiname}"});
    delete($actionees{"$Foswiki::cfg{UsersWebName}.$this->{test_user_wikiname}"});
...
sub testAllActionsInWebMain {
    ...
    my $actions = Foswiki::Plugins::ActionTrackerPlugin::ActionSet::allActionsInWeb($this->{test_web_2}, $attrs, 0);
    my $fmt = $textonlyfmt;
    my $chosen = $actions->formatAsString( $fmt );
