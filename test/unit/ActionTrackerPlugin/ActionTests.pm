...
sub test_NewNoState {
    ...
    $action =
      new Foswiki::Plugins::ActionTrackerPlugin::Action(
          "Test","Topic",0,"closed=10-may-05", "");
    $this->assert_str_equals("closed", $action->{state});
    $action =
      Foswiki::Plugins::ActionTrackerPlugin::Action->new(
...
sub test_IsLateAndDaysToGo {
    ...
    $this->assert_num_equals(-2, $action->daysToGo());
    $action = new
      Foswiki::Plugins::ActionTrackerPlugin::Action(
          "Test","Topic",0,
...
sub test_MatchesOpen {
    ...
    my $action =
      new Foswiki::Plugins::ActionTrackerPlugin::Action(
          "Test", "Topic", 0,
          'who="JohnDoe,SlyStallone,'.$Foswiki::cfg{DefaultUserWikiName}.'" due="2 Jun 02" state=open notify="SamPeckinpah,QuentinTarantino" created="1999-01-01" creator="ThomasMoore"',
          "A new action");
...
sub test_StringFormattingClosed {
    ...
    my $s = $fmt->formatStringTable([$action]);
    $this->assert_str_equals(
        $s, "|01 Jan 2003|$this->{users_web}.LucBesson|\n");
}
...
sub test_VerticalOrient {
    ...
    $s =~ s/<\/table>//;
    $s =~ s/\n//g;
    $this->assert_str_equals("<tr><th align=\"right\">Who</th><td><a name=\"AcTion0\" />$this->{users_web}.JohnDoe</td></tr><tr><th align=\"right\">Due</th><td><span class=\"atpOpen\">02 Jun 2002</span></td></tr>", $s);
}
...
sub test_HTMLFormattingOpen {
    ...
    my $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format("", "|\$who|");
    my $s = $fmt->formatHTMLTable([$action], "name", 0, 'atp');
    ...
    $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format("", "| \$notify |");
    $s = $fmt->formatHTMLTable([$action], "href", 0, 'atp');
    $this->assert_html_matches(
    ...
    $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format(
        "", "| \$edit |", "");
    my $url = "$Foswiki::cfg{DefaultUrlHost}$Foswiki::cfg{ScriptUrlPath}/edit$Foswiki::cfg{ScriptSuffix}/Test/Topic\\?skin=action%2cpattern;atp_action=AcTion0;t=";
    ...
    $this->assert( $s =~ /<td> A new action <a href="$Foswiki::cfg{DefaultUrlHost}$Foswiki::cfg{ScriptUrlPath}\/view$Foswiki::cfg{ScriptSuffix}\/Test\/Topic#AcTion0">/,
        $s );
...
sub test_AutoPopulation {
    ...
    my $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format(
        "","","","|\$uid|\$who|");
    my $s = $fmt->formatStringTable([$action]);
    $this->assert_matches(qr/^\|\d\d\d\d\d\d\|$this->{users_web}\.$Foswiki::cfg{DefaultUserWikiName}\|\n/, $s);
    $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format(
        "","","","|\$creator|\$created|");
    $s = $fmt->formatStringTable([$action]);
    $this->assert_str_equals("|$this->{users_web}.$Foswiki::cfg{DefaultUserWikiName}|31 May 2002|\n", $s);
    $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format(
        "","","","|\$closer|\$closed|");
    $s = $fmt->formatStringTable([$action]);
    $this->assert_str_equals("|$this->{users_web}.$Foswiki::cfg{DefaultUserWikiName}|31 May 2002|\n", $s);
...
sub test_ToString {
    ...
    $s =~ s/ creator="$this->{users_web}\.$Foswiki::cfg{DefaultUserWikiName}"//o;
...
sub test_FindChanges {
    ...
    my $naction = Foswiki::Plugins::ActionTrackerPlugin::Action->new( "Test", "Topic", 0, "who=JaneDoe due=\"2 Jun 09\" state=closed notify=\"SamPeckinpah,QuentinTarantino\" creator=\"ThomasMoore\"", "A new action<p>with more text");
    ...
    $this->assert_matches(
        ...
        $text);
    $this->assert_matches(
        qr/Attribute \"created\" was \"1 Jan 1999\" now removed/,
        $text);
    $this->assert_matches(
...
sub test_XtendTypes {
    ...
    $s = Foswiki::Plugins::ActionTrackerPlugin::Action::extendTypes("|fleegle|");
...
sub test_FormatForEditHidden {
    ...
    my $action =
      new Foswiki::Plugins::ActionTrackerPlugin::Action(
          ...
          "Text");
    my $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format( "|Who|", "|\$who|", "cols","","");
...
sub test_FormatForEdit {
    ...
    Foswiki::Plugins::ActionTrackerPlugin::Action::forceTime("31 May 2002");
    $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format( "|Due|", "|\$due|", "","");
...
sub test_ExtendStates {
    ...
    my $s = Foswiki::Plugins::ActionTrackerPlugin::Action::extendTypes("|state,select,17,life,\"5 years\",\"community service\"|");
    $this->assert(!defined($s));
    my $action =
      new Foswiki::Plugins::ActionTrackerPlugin::Action("Web", "Topic", 10,
                                                      "state=\"5 years\"", "Text");
    my $fmt = new Foswiki::Plugins::ActionTrackerPlugin::Format( "|State|", "|\$state|", "","");
    $s = $action->formatForEdit($fmt);
